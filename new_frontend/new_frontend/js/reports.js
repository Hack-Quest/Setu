// API Service to match existing Python backend structure
...
async function fetchReports() {
    try {
        const response = await fetch(`${API_BASE_URL}/dashboard/reports`, {
            method: 'GET',
            headers: {
                // Incorporating auth structure from api.js if needed
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('auth_token') || 'hackathon-secret'}`
            }
        });
    ...
    if (reports.length === 0) {
        ...
        reports.forEach((report, index) => {
            ...
            if (safePriority === "HIGH") {
                ...
                priorityBadgeStyle = "bg-amber-50 text-amber-700 border border-amber-200/50";
