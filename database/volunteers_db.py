...
def register_volunteer_auth(email: str, password: str, name: str, phone: str, location: str, skills: list) -> dict:
    ...
    existing = db.collection("volunteers_auth").where("email", "==", email).stream()
    if list(existing):
    ...
    volunteer_data = {
        ...
        "phone": phone,
        "location": location,
        "skills": [s.lower().strip() for s in skills] if isinstance(skills, list) else [skills.lower().strip()],
...
def login_volunteer(email: str, password: str) -> dict:
    ...
    docs = db.collection("volunteers_auth").where("email", "==", email).stream()
