...
def run_test():
    ...
    db.collection("volunteers").document(tier1_id).update({"lat": 28.6100, "lng": 77.2000})
    db.collection("volunteers").document(tier2_id).update({"lat": 28.6120, "lng": 77.2085})
    ...
    docs = db.collection("assignments").where("need_id", "==", need_id).stream()
    ...
    if len(assignments) == 0:
        ...
        if assigned_vol_id == tier1_id:
            print("✅ SUCCESS: Tier 1 Medic was correctly chosen over closer Tier 2 Medic for sensitive 'Medical' SOS.")
